...
import com.yourname.icepacklist.feature.home.domain.PersonDetailFullResponse
...
class DetailRepository @Inject constructor(
    ...
    suspend fun getPersonDetailFull(personId: Int): Result<PersonDetailFullResponse> {
        val adapter = moshi.adapter(PersonDetailFullResponse::class.java)
        val cached = personCacheDao.get(personId)
        if (cached != null && !CacheConfig.isStale(cached.fetchedAt)) {
            val fromCache = runCatching { adapter.fromJson(cached.json) }.getOrNull()
            if (fromCache != null) return Result.success(fromCache)
        }
        return runCatching {
            withContext(Dispatchers.IO) {
                apiService.getPersonDetailFull(personId).also { detail ->
                    personCacheDao.upsert(
                        com.yourname.icepacklist.core.database.entity.PersonCacheEntity(
                            personId, adapter.toJson(detail), System.currentTimeMillis()
                        )
                    )
                }
            }
